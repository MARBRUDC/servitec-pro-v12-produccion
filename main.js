...
function render(){app.innerHTML=`<header class="top"><div><h1>SERVITEC PRO V13.12.1 PDF CORPORATIVO</h1><b>Cotización dinámica + Nube + PDFs corporativos</b></div><div class="tag">${cloud}</div></header><nav class="nav">${['Dashboard','Empresas','Clientes','Cotizaciones','Ejecución','Actas','Informes','Inventario','Configuración'].map(x=>`<button class="${tab===x?'active':''}" onclick="tab='${x}';draft=null;render()">${x}</button>`).join('')}</nav><main>${views[tab]()}</main>`; bind()}
...
function quoteDoc(q){
  ...
  <div class="cond"><h3>CONDICIONES COMERCIALES</h3><b>Moneda:</b> Soles (S/).<br><b>Forma de pago:</b> Según coordinación / contado comercial.<br><b>Tiempo de ejecución:</b> Según programación y disponibilidad del área usuaria.<br><b>Lugar del servicio:</b> En establecimiento del cliente.<br><b>Validez de oferta:</b> 15 días calendario.<br><b>Garantía:</b> Según alcance del servicio ejecutado.<div class="obs"><b>Observaciones:</b><br>• Incluye mano de obra especializada.<br>• No incluye repuestos, consumibles o trabajos adicionales no descritos.<br>• Toda actividad adicional será cotizada por separado.</div></div>
  <p>Sin otro particular, quedamos de ustedes.</p>
  <div class="firma"><div class="line">GERENTE GENERAL<br>${emp.nombre||'SERVITEC PRO'}</div></div>
