...
function render(){app.innerHTML=`<header class="top"><div><h1>SERVITEC PRO V13.15 EJECUCIÓN REAL</h1><b>Cotización dinámica + Nube + PDFs corporativos</b></div><div class="tag">${cloud}</div></header><nav class="nav">${['Dashboard','Empresas','Clientes','Cotizaciones','Ejecución','Actas','Informes','Inventario','Configuración'].map(x=>`<button class="${tab===x?'active':''}" onclick="tab='${x}';draft=null;render()">${x}</button>`).join('')}</nav><main>${views[tab]()}</main>`; bind()}
...
function execRowsBase(q){
  if(q.config===MODES.PROPIAS){
    return (q.equipos||[]).flatMap(e=>(e.acts||[]).map(a=>({id:a.id||uid(),equipo:e.nombre||'Equipo biomédico',descripcion:a.descripcion,cantidad:a.cantidad||1,precio:a.precio||0})));
  }
  if(q.config===MODES.REPUESTOS){
    return (q.repuestos||[]).map(r=>({id:r.id||uid(),equipo:'Repuesto / accesorio',descripcion:r.descripcion,cantidad:r.cantidad||1,precio:r.precio||0}));
  }
  return (q.acts||[]).map(a=>({id:a.id||uid(),equipo:(q.equipos||[]).map(e=>e.nombre).filter(Boolean).join(', ')||'Equipos incluidos',descripcion:a.descripcion,cantidad:a.cantidad||1,precio:a.precio||0}));
}
