...
function rowAmount(row) {
  const qtyRaw = row?.cantidad ?? row?.qty ?? 1;
  const priceRaw = row?.precio ?? row?.costo ?? row?.price ?? 0;
  const quantity = toNumber(qtyRaw) || 1;
  const price = toNumber(priceRaw);
  return quantity * price;
}

function getAllQuoteRows(quote) {
  const rows = [];

  (quote.equipos || []).forEach((equipment) => {
    (equipment.actividades || []).forEach((activity) => rows.push(activity));
  });

  (quote.actividadesGenerales || []).forEach((activity) => rows.push(activity));
  (quote.repuestos || []).forEach((item) => rows.push(item));

  return rows;
}

function calculateTotal(quote) {
  const rows = getAllQuoteRows(quote);
  let total = 0;

  rows.forEach((row) => {
    const quantity = Number(row?.cantidad || 1) || 1;
    const price = Number(
      String(row?.precio ?? row?.costo ?? 0)
        .replace("S/", "")
        .replace(",", ".")
        .replace(/[^0-9.-]/g, "")
    ) || 0;
...
function App() {
  ...
  function go(id) {
    ...
    if (deltaX > 110 && deltaY < 70 && canGoBack()) {
      goBack();
    }
  }
    ...
    <div className="shell">
      ...
      <aside className={sidebarOpen ? "sidebar open" : "sidebar"}>
        ...
        <div className="brand">
          ...
          <div>
            <b>SERVITEC PRO V11.9</b>
            <small>Responsive real</small>
          </div>
        </div>
      ...
      <main className="main" onFocusCapture={enableFieldModeProtection}>
        <div className="mobileTopbar">
          ...
          <div>
            ...
            <small>V11.9 PDF TOTAL FIX</small>
          </div>
        </div>
...
function Dashboard({ scoped }) {
  return (
    <>
      ...
      <Card title="Flujo V11.9 PDF TOTAL FIX">
        <ol>
...
function Cotizaciones({ data, empresa, scoped, saveData }) {
  ...
  function saveQuote() {
    ...
    const fixedTotalBase = calculateTotal(quote);
    const fixedTotal = fixedTotalBase * 1.18;
    const item = { ...quote, totalBase: fixedTotalBase, total: fixedTotal, fecha: quote.fecha || today() };
  ...
  return <Card title="Cotizaciones"><div style={{background:"#dff7e8",padding:"10px",borderRadius:"12px",fontWeight:"800",marginBottom:"10px"}}>SERVITEC PRO V11.9 · TOTAL Y PDF ACTIVO</div><div className="versionBanner">SERVITEC PRO V11.9 · TOTAL Y PDF ACTIVO</div>
    {mode === "list" && <>
...
function QuoteDocument({ empresa, quote, scoped }) {
  const rowsForTotal = getAllQuoteRows(quote);
  const documentTotalBase = rowsForTotal.reduce((sum, row) => sum + rowAmount(row), 0);
  const documentIgv = documentTotalBase * 0.18;
  const documentTotal = documentTotalBase + documentIgv;
  ...
  return (
    <div>
      ...
      <div className="quoteMode">
        <b>Configuración:</b> {structureLabel(quote.estructura)} · <b>Ítems cotizados:</b> {rowsForTotal.length}
      </div>

      {quote.estructura === "EQUIPO_ACTIVIDADES" && (quote.equipos || []).map((equipment, index) => (
        <div className="docBox" key={equipment.id}>
          <h3>Equipo {index + 1}: {equipment.nombre || "-"}</h3>
          <p>Marca: {equipment.marca || "-"} · Modelo: {equipment.modelo || "-"} · Serie: {equipment.serie || "-"} · Código: {equipment.codigoPatrimonial || "-"}</p>
          <SimpleRows rows={equipment.actividades || []} />
        </div>
      ))}
      ...
      {quote.estructura === "ACTIVIDADES_GENERALES_EQUIPOS" && (
        <>
          ...
          <div className="docBox">
            ...
            <table>
              <thead>
                <tr><th>#</th><th>Equipo</th><th>Marca</th><th>Modelo</th><th>Serie</th><th>Código</th></tr>
              </thead>
              <tbody>
                {(quote.equiposGenerales || []).length > 0 ? (quote.equiposGenerales || []).map((equipment, index) => (
                  <tr key={equipment.id}>
                    <td>{index + 1}</td>
                    ...
                    <td>{equipment.codigoPatrimonial}</td>
                  </tr>
                )) : (
      ...
      <h3 className="finalTotalBox">
        Total base: {money(documentTotalBase)} · IGV: {money(documentIgv)} · Total: {money(documentTotal)}
      </h3>
...
function SimpleRows({ rows }) {
  return (
    <table>
      <thead>
        <tr><th>#</th><th>Descripción</th><th>Cantidad</th><th>Precio</th><th>Subtotal</th></tr>
      </thead>
      <tbody>
        {rows.length > 0 ? rows.map((row, index) => (
          <tr key={row.id || index}>
            ...
            <td>{toNumber(row.cantidad || 1)}</td>
            <td>{money(row.precio)}</td>
            <td>{money(rowAmount(row))}</td>
          </tr>
        )) : (
          <tr><td colSpan="5">Sin detalle registrado.</td></tr>
        )}
      </tbody>
    </table>
  );
...
function Ejecucion({ data, empresa, scoped, saveData }) {
  ...
  function save() {
    ...
    const item = { id: uid("OS"), empresaId: empresa.id, cotizacionId: quote.id, numero: `${empresa.serieOS}-${String(scoped.ejecuciones.length + 1).padStart(4, "0")}`, fecha: today(), tecnico, estado, trabajos: jobs.map(j => ({ ...j, observaciones: obs[j.id] || "" })) };
    saveData({ ...data, ejecuciones: [...data.ejecuciones, item], cotizaciones: data.cotizaciones.map(q => q.id === quote.id ? { ...q, estado: "En ejecución" } : q) }, "Ejecución guardada");
    alert("Ejecución guardada");
  }

  return <Card title="Ejecución ligada a cotización">
    <Select label="Cotización" value={quoteId} onChange={setQuoteId}><option value="">Seleccione cotización</option>{scoped.cotizaciones.map(q => <option key={q.id} value={q.id}>{q.numero} · {findName(scoped.clientes, q.clienteId, "razonSocial")}</option>)}</Select>
    <div className="grid3"><Input label="Técnico responsable" value={tecnico} onChange={setTecnico} /><Select label="Estado" value={estado} onChange={setEstado}><option>En ejecución</option><option>Observado</option><option>Finalizado</option></Select></div>
    <div className="jobList">{jobs.map((j, i) => <div className="job" key={j.id}><b>{i + 1}. {j.titulo}</b><span>{j.detalle}</span><textarea placeholder="Fecha, estado, observaciones y evidencias" value={obs[j.id] || ""} onChange={e => setObs({ ...obs, [j.id]: e.target.value })} /></div>)}</div>
    <button onClick={save}><Save size={16} /> Guardar ejecución</button>
    <Table rows={scoped.ejecuciones} cols={[["numero", "OS"], ["fecha", "Fecha"], ["tecnico", "Técnico"], ["estado", "Estado"]]} />
  </Card>;
}
function extractJobs(q) {
  if (q.estructura === "EQUIPO_ACTIVIDADES") return (q.equipos || []).flatMap((eq, ei) => (eq.actividades || []).map((a, ai) => ({ id: a.id, titulo: `${eq.nombre || "Equipo " + (ei + 1)} · ${a.descripcion || "Actividad " + (ai + 1)}`, detalle: `Serie: ${eq.serie || "-"} · Código: ${eq.codigoPatrimonial || "-"}` })));
  if (q.estructura === "ACTIVIDADES_GENERALES_EQUIPOS") return (q.actividadesGenerales || []).map((a, i) => ({ id: a.id, titulo: `Actividad general ${i + 1}: ${a.descripcion || "-"}`, detalle: `Aplica a ${(q.equiposGenerales || []).length} equipos.` }));
  return (q.repuestos || []).map((r, i) => ({ id: r.id, titulo: `Repuesto/producto ${i + 1}: ${r.descripcion || "-"}`, detalle: `Cantidad: ${r.cantidad || 0}` }));
}
